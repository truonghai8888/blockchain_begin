...
from blockchain import SendTransaction, blockchain
from PySide2.QtCore import SIGNAL, QSize, Qt, Slot
from PySide2.QtGui import QColor, QMovie, QPalette, QPixmap
...
class SendWidget(QWidget):
    ...
    def updateBalanceLabel(self, token_name, account_address, token_information=None):
        if token_name == "Ethereum":
            ...
            self.balance_label.setText(
                "Balance: %d coins" % blockchain.get_token_balance(account_address, token_information)
            )
    ...
    def sendButtonClicked(self):
        password, ok = QInputDialog.getText(self, "Create A New Transaction", "Password:", QLineEdit.Password)
        if ok and password != "":
