...
erc20_token_interface = [
    ...
    {"constant": false, "inputs": [], "name": "__init__", "outputs": [], "payable": false, "type": "constructor"},
    {
        "constant": true,
        "gas": 655,
...
class Blockchain:
    ...
    def get_accounts(self):
        return map(
            lambda account: (account, self.w3.fromWei(self.w3.eth.getBalance(account), "ether")), self.w3.eth.accounts
        )
    ...
    def create_send_token_transaction(self, tx, token_information):
        ...
        transaction = token_contract.functions.transfer(tx.destination, int(tx.amount)).buildTransaction(
            {"from": tx.sender, "gas": 70000, "gasPrice": self.w3.toWei(str(tx.fee), "gwei"), "nonce": nonce}
        )
    ...
    def get_information_of_token(self, address):
        ...
        token_information = TokenInformation(
            name=name.decode("utf-8"), symbol=symbol.decode("utf-8"), totalSupply=total_supply, address=address
        )
