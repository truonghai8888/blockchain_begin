...
def test_purchase_token(web3, chain):
    ...
    set_txn_hash = crowd_sale_token.functions.__default__().transact(
        {"from": account2, "value": web3.toWei("1", "ether")}
    )
    chain.wait.for_receipt(set_txn_hash)

    set_txn_hash = crowd_sale_token.functions.__default__().transact(
        {"from": account3, "value": web3.toWei("2", "ether")}
    )
    chain.wait.for_receipt(set_txn_hash)
...
def test_refund(web3, chain):
    ...
    set_txn_hash = crowd_sale_token.functions.__default__().transact(
        {"from": account2, "value": web3.toWei("5", "ether")}
    )
    chain.wait.for_receipt(set_txn_hash)
    ...
    assert abs(
        beforeCrowdsaleEthBalanceAccount2 - web3.toWei("5", "ether") - afterCrowdsaleEthBalanceAccount2
    ) < web3.toWei("1", "gwei")
...
def test_withdrawal(web3, chain):
    ...
    set_txn_hash = crowd_sale_token.functions.__default__().transact(
        {"from": account2, "value": web3.toWei("40", "ether")}
    )
    chain.wait.for_receipt(set_txn_hash)
    ...
    assert abs(
        beforeCrowdsaleEthBalanceAccount2 - afterCrowdsaleEthBalanceAccount2 - web3.toWei("40", "ether")
    ) < web3.toWei("1", "gwei")
    assert abs(
        afterCrowdsaleEthBalanceAccount1 - beforeCrowdsaleEthBalanceAccount1 - web3.toWei("40", "ether")
    ) < web3.toWei("1", "gwei")
